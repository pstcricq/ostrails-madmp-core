...
Two things are worth testing here, and the second is the reason the module
exists. First, that a real config comes back as the answers every generator
will ask of it. Second, that it is all of them or none: a project whose
resources do not all resolve must not come back holding the ones that did.
"""
...
def test_the_model_carries_the_versions_the_config_pins():
    """The provenance a QC run reproduces: the merged model carries the very
    versions the config named, in the order it named them, not whatever is
    latest on disk."""
    project = assemble_project(GLIDER_CONFIG)
...
def test_the_config_comes_back_unchanged(tmp_path):
    """The door composes, it does not reshape: what `configs/` validated is
    what a generator reads, field for field."""
    project = assemble_project(GLIDER_CONFIG)
    assert project.config == yaml.safe_load(GLIDER_CONFIG.read_text())


def test_a_project_is_frozen():
    """The value travels into every builder; none of them gets to edit it for
    the others."""
    project = assemble_project(GLIDER_CONFIG)
...
def test_a_missing_rules_version_stops_the_load(tmp_path):
    ...
    well; the project is still broken, and no caller gets to see half of it."""
...
def test_a_malformed_config_stays_the_config_package_s_error(tmp_path):
    """Every step keeps its own error type. The door adds none of its own and
    wraps none of theirs: whoever is handed the failure is told which layer
    found it."""
    doc = yaml.safe_load(GLIDER_CONFIG.read_text())
    del doc["rules"]
