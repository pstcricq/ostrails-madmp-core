...
# The mono-repo every project's folder lives in. Overridable so that a test,
# or somebody else's deployment, points elsewhere.
REGISTRY_OWNER = os.environ.get("REGISTRY_OWNER", "Pierrott64")
...
def token_from_env() -> str:
    """The token, or an empty string. ``REGISTRY_TOKEN`` is the name the whole
...
def _read_meta(gh: GitHubClient, config: dict[str, Any]) -> tuple[Any, dict | None]:
    """The file and the document it parses to, or ``(None, None)``."""
    entry = gh.get_file(REGISTRY_OWNER, REGISTRY_REPO, meta_path(config))
    if entry is None:
        return None, None
    return entry, yaml.safe_load(entry.content)


def folder_status(gh: GitHubClient, config: dict[str, Any]) -> FolderStatus:
    """Read this project's folder and say where it stands. Writes nothing."""
    folder = config["id"]
    _, current = _read_meta(gh, config)
    if current is None:
        return FolderStatus(
...
def converge(gh: GitHubClient, config: dict[str, Any]) -> str:
    ...
    """
    entry, current = _read_meta(gh, config)
    ...
    for sub in SUBDIRS:
        ...
        if gh.get_file(REGISTRY_OWNER, REGISTRY_REPO, keep) is None:
            gh.put_file(
                REGISTRY_OWNER,
                REGISTRY_REPO,
                keep,
